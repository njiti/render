...
pub mod zwp_fullscreen_shell_v1 {
    ...
    pub trait Handler {
        ...
        fn capability(&mut self, evqh: &mut EventQueueHandle,  proxy: &ZwpFullscreenShellV1, capability: u32) {}
        #[doc(hidden)]
        unsafe fn __message(&mut self, evq: &mut EventQueueHandle,  proxy: &ZwpFullscreenShellV1, opcode: u32, args: *const wl_argument) -> Result<(),()> {
            match opcode {
                0 => {
...
pub mod zwp_fullscreen_shell_mode_feedback_v1 {
    ...
    pub trait Handler {
        ...
        fn mode_successful(&mut self, evqh: &mut EventQueueHandle,  proxy: &ZwpFullscreenShellModeFeedbackV1) {}
        /// mode switch failed
        ///
        ...
        /// Upon receiving this event, the client should destroy the
        /// wl_fullscreen_shell_mode_feedback object.
        fn mode_failed(&mut self, evqh: &mut EventQueueHandle,  proxy: &ZwpFullscreenShellModeFeedbackV1) {}
        ...
        /// Upon receiving this event, the client should destroy the
        /// wl_fullscreen_shell_mode_feedback object.
        fn present_cancelled(&mut self, evqh: &mut EventQueueHandle,  proxy: &ZwpFullscreenShellModeFeedbackV1) {}
        #[doc(hidden)]
        unsafe fn __message(&mut self, evq: &mut EventQueueHandle,  proxy: &ZwpFullscreenShellModeFeedbackV1, opcode: u32, args: *const wl_argument) -> Result<(),()> {
            match opcode {
                0 => {
                    self.mode_successful(evq,  proxy);
                },
                1 => {
                    self.mode_failed(evq,  proxy);
                },
                2 => {
                    self.present_cancelled(evq,  proxy);
                },
                _ => return Err(())
            }
            Ok(())
